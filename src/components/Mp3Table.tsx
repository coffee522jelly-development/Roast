...
export function Mp3Table({
  ...
  const renderQualityIcon = (bitrate: number | null) => {
    ...
    if (bitrate >= 320) return <span title="High Quality" className="flex items-center"><Signal className="h-3 w-3 text-primary opacity-60" /></span>;
    if (bitrate >= 192) return <span title="Medium Quality" className="flex items-center"><SignalMedium className="h-3 w-3 text-muted-foreground opacity-60" /></span>;
    return <span title="Low Quality" className="flex items-center"><SignalLow className="h-3 w-3 text-muted-foreground/40" /></span>;
  };
